...
namespace FriendlyWinnerUnitTest
{
    ...
    public class UnitTest
    {
        ...
        public void DontRepeatToFast()
        {
            ...
            Assert.AreEqual(
                false,
                FriendlyWinnerDll.FriendlyWinner.RepeatMaximum(
                    20, FriendlyWinnerDll.FriendlyWinner.Motivation.GameStart));
        }
            ...
            for (var i = 0; i < FriendlyWinnerDll.FriendlyWinner.Motivation.GameStart.Count; i++)
            {
                list.Add(
                    FriendlyWinnerDll.FriendlyWinner.GetNewMessage(
                        FriendlyWinnerDll.FriendlyWinner.GetMessage,
                        FriendlyWinnerDll.FriendlyWinner.Motivation.GameStart));
            }
        ...
        private static void AllMessagesUsedGetNext(IEnumerable<string> list)
        {
            ...
            {
                var match =
                    FriendlyWinnerDll.FriendlyWinner.Motivation.GameStart.FirstOrDefault(x => x.Message.Equals(result));
